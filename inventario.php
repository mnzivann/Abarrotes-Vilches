...
session_start();
// ============================================================================
// LÓGICA DE INVENTARIO (SIMULACIÓN DE BASE DE DATOS)
// ============================================================================

// Simulación de Productos y su Stock actual (RF_07: Consultar)
$productosBD = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Buscar el stock actual del producto simulado
    $stock_actual = 0;
    foreach($productosBD as $p) { if($p['id'] == $id_producto) { $stock_actual = $p['stock']; break; } }

    // [RF_05] ENTRADA DE INVENTARIO
    if ($accion === 'entrada') {
        if ($cantidad > 0) {
            // Aquí Jacobo hará: UPDATE Productos SET stock = stock + ? WHERE id = ?
            // Y luego: INSERT INTO MovimientosInventario (tipo, cantidad, ...) VALUES ('Entrada', ?, ...)
            $mensaje = "<div class='alert alert-success'>Entrada registrada: Se agregaron $cantidad unidades.</div>";
        } else {
            $mensaje = "<div class='alert alert-danger'>Error: La cantidad de entrada debe ser mayor a 0.</div>";
        }
    }

    // [RF_06] SALIDA DE INVENTARIO
    if ($accion === 'salida') {
        if ($cantidad > 0 && $stock_actual >= $cantidad) {
            // Aquí harán: UPDATE Productos SET stock = stock - ? WHERE id = ?
            $mensaje = "<div class='alert alert-warning'>Salida registrada: Se retiraron $cantidad unidades.</div>";
        } else if ($cantidad > $stock_actual) {
            $mensaje = "<div class='alert alert-danger'>Error: No hay suficiente stock. (Disponible: $stock_actual).</div>";
        } else {
            $mensaje = "<div class='alert alert-danger'>Error: La cantidad debe ser mayor a 0.</div>";
        }
    }
}
?>
